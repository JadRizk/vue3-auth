...
import { addAuthHeader, loginService, logoutService, registerService } from 'api/auth.services';
...
export type AuthMutation<S = State> = {
    ...
    [AuthMutationType.AUTH_FAILURE](state: S, payload: { error: storeErrorT }): void;
    [AuthMutationType.SET_TOKEN](state: S, payload: { token: string }): void;
    [AuthMutationType.REGISTER_SUCCESSFUL](state: S, payload: { email: string }): void;
    [AuthMutationType.LOGOUT](state: S, payload: {}): void;
}

export const AuthMutations: MutationTree<State> & AuthMutation = {
    [AuthMutationType.LOGIN_SUCCESSFUL]({ auth }: State, payload: { user: User; token: string }) {
        auth.isLoggedIn = true;
        auth.user = payload.user;
    },
    [AuthMutationType.AUTH_FAILURE]({ auth }: State, payload: { error: storeErrorT }) {
    ...
    [AuthMutationType.REGISTER_SUCCESSFUL]({ auth }: State, payload: { email: string }) {
...
export const authActions: ActionTree<State, State> & AuthActions = {
    async [AuthActionTypes.LOGIN]({ commit }, userDetails: { username: string; password: string }) {
        ...
        return await loginService(username, password)
            .then((response) => {
                ...
                if (error.response.status === 422) {
                    ...
                    commit(AuthMutationType.AUTH_FAILURE, { error: { message: errorMessage, type: 'error' } });
    ...
    async [AuthActionTypes.REGISTER]({ commit }, userDetails: RegisterUserInfo) {
        ...
        return await registerService(userDetails)
            .then((response) => {
                // Deconstruct the payload 🪚            
                const { data: { player: { email } } } = response;
